...
void initStruct(void) {
    ...
    //Rajout de ce mutex compteur de perte de message
    if (err = rt_mutex_create(&mutexCompteurRecep, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_mutex_create(&mutexInitConnexion, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_mutex_create(&mutexAreneCam, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
   ...
   /* if (err = rt_mutex_create(&mutexComRobot, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }*/
    ...
    if (err = rt_sem_create(&semArene, NULL, 0, S_FIFO)) {
        rt_printf("Error semaphore create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_sem_create(&semPhoto, NULL, 0, S_FIFO)) {
        rt_printf("Error semaphore create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
   /* Creation de l'event */
    ...
    if (err = rt_task_create(&tarena, NULL, 0, PRIORITY_TARENE, 0)) {
        rt_printf("Error task create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_create(&tphoto, NULL, 0, PRIORITY_TPHOTO, 0)) {
        rt_printf("Error task create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
...
void startTasks() {
    ...
    if (err = rt_task_start(&tcoperdue, &connexion_perdue, NULL)) {
     ...
     if (err = rt_task_start(&tarena, &th_arene, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_start(&tphoto, &photo, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
